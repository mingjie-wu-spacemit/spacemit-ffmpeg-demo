/*
 * SpaceMIT MJPEG Hardware Decoding Demo
 *
 * Demonstrates hardware-accelerated MJPEG decoding using mjpeg_stcodec decoder.
 */
...
int main(int argc, char *argv[]) {
    if (argc < 2) {
        ...
        fprintf(stderr, "Example: %s motion.mjpeg 100\n", argv[0]);
        fprintf(stderr, "         %s image.jpg\n", argv[0]);
        return 1;
    }
    ...
    // Find MJPEG hardware decoder
    decoder = avcodec_find_decoder_by_name("mjpeg_stcodec");
    if (!decoder) {
        fprintf(stderr, "mjpeg_stcodec decoder not found, trying default mjpeg decoder\n");
        decoder = avcodec_find_decoder(AV_CODEC_ID_MJPEG);
        if (!decoder) {
            fprintf(stderr, "No MJPEG decoder available\n");
            ret = -1;
            goto cleanup;
        }
    } else {
        printf("Using hardware decoder: mjpeg_stcodec\n");
    }
